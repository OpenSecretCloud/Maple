...
The bundle guarantees the Python standard library, not project packages. The task root is available for explicit project imports. Never install into the shared runtime or app bundle. When pip is available (portable bundle; not guaranteed in Nix), invoke sys.executable with -I -B -m pip --isolated install --only-binary=:all: --target /absolute/external/dependency/directory, then explicitly add that directory to sys.path. Keep -B to avoid writing import caches into the runtime. Use a separate environment if pip or a compatible wheel is unavailable. Creating or activating a venv in a shell does not retarget this retained worker. Spawned process pools require functions from importable modules; functions defined only in cells can fail or hang a pool. There is no Maple Python SDK yet.

Errors preserve partial assignments and external effects. Output is bounded and truncation is reported; use files for large results. Synchronous blocking calls are allowed but pause other asyncio work for their duration. Stop retires an unfinished Python cell and loses its namespace. Synchronous code can continue producing external effects during the retirement grace period; force termination can skip cleanup handlers and lose buffered output. Stop after a settled cell preserves its namespace and background work. Reset explicitly ends retained Python work. Set reset=true to start fresh before supplied code, or pass code="" with reset=true to release this task's worker without starting another. App restart and task/owner retirement also discard state. This is ordinary native Python execution with the same machine access as other execution tools."#,
        object!({
            "type": "object",
