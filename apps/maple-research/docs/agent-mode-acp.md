...
| Project trust                        | Yes           | Resolved before the first real prompt when protected project guidance is present; never during `session/new`.      |
...
## Generic project trust

Project trust is one reversible, account-scoped decision for a canonical project directory. It is intentionally not split into separate prompts for skills, instructions, or future project-provided capabilities. Skills are the first protected capability: when trust is disabled or undecided, Maple's skills client is rooted in an owner-only empty directory, so project `.agents/skills/**/SKILL.md` files are neither discovered nor injected into the model context. This is a policy boundary, not a filesystem sandbox; ordinary read or shell tools retain their separately approved filesystem authority.

Maple prompts only when the project currently contains a real, non-symlinked `SKILL.md` under `.agents/skills` and the canonical project has no saved decision. Desktop checks when the project is selected and again at each task boundary, so a skill added after the folder was opened is discovered on the next new or selected task without polling or interrupting an already-open task. ACP deliberately does **not** ask during `session/new`, because clients such as Paseo create disposable sessions for model discovery. Instead, Maple resolves trust immediately before the first real `session/prompt`, before the user message reaches the model:

1. if the ACP client advertises form elicitation, Maple sends a session-scoped `elicitation/create` request whose required boolean defaults to false;
2. otherwise Maple sends a permission chooser with `Keep project trust disabled` first, `Trust this project` second, and `Cancel turn` as the rejecting choice; and
3. after either decision, Maple persists it and rebuilds the leased session's derived Agent catalog before admitting the user prompt.

The two same-kind allow choices are intentional for Paseo: they force a visible chooser instead of allowing Paseo's generic Auto Accept feature to silently decide trust. Declining an elicitation means keep trust disabled; cancellation cancels the pending turn without saving a decision. If a client advertises elicitation but cannot complete it, Maple falls back to the same chooser.

Desktop exposes the durable switch from each Agent project menu under **Project Settings**. The switch can turn trust on or off later; Maple rejects changes while that project's tasks are running or externally leased, then unloads idle cached Agents so the next run observes the new decision. Existing `projectSkillsTrust` configuration migrates through a serde alias and is written back as the generic `projectTrust` field.

When an untrusted model nevertheless calls `load_skill`, Maple returns an explicit error that no **trusted** skill was found, tells the model to ask the user to trust the project or continue without it, and tells it not to read project skill files directly unless the user explicitly asks. Trusted and global skills retain the ordinary Goose behavior.
...
| Project trust             | Verify `session/new` and model discovery never prompt. On the first real prompt in a project with `.agents/skills/**/SKILL.md`, verify form elicitation when advertised and the fail-closed three-choice Paseo fallback otherwise; both trust choices persist, the original prompt continues only after the decision, and cancel admits no user turn. In Desktop, open a project without skills, add a skill, and verify the next new or selected task prompts without interrupting the current task. Toggle trust in Desktop Project Settings and verify the next idle run adds/removes project skills. |
...
```bash
nix develop --no-update-lock-file -c apps/maple-research/frontend/src-tauri/scripts/run-with-desktop-onnxruntime.sh \
  cargo test --locked --manifest-path apps/maple-research/frontend/src-tauri/Cargo.toml agent_acp --lib
```
