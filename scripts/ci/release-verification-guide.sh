...
The release contains final SHA-256 manifests, canonical payload manifests, Tauri updater signatures, and GitHub artifact attestations. Together they let you check that downloaded bytes match the release, that updater signatures match \`latest.json\`, and that signed Android, macOS, and iOS payloads strip back to the reproducible unsigned build products where that platform supports the check. Windows release proofs cover the final Authenticode-signed NSIS installer, its final Tauri updater signature, the pinned app-local runtime DLLs used during bundling, and the signed installer's canonical app/runtime payload after excluding the NSIS-generated uninstaller helper. The proxy proof covers the four native proxy archives and verifies that each contains only the expected executable.

GitHub attestations prove that release CI produced the assets explicitly attested by their build jobs for this tag. The Maple verifier then recomputes the local hashes and canonical payload proofs from the files you downloaded.
...
For example, the proxy publisher attests every proxy archive and its checksum
manifest, so you can verify them with:

\`\`\`bash
for file in artifacts/maple-proxy-*; do
...
# Native maple-proxy archives and their unified checksum manifest.
nix develop --no-update-lock-file .#ci -c ./scripts/ci/verify-release-artifacts.sh artifacts proxy
\`\`\`
