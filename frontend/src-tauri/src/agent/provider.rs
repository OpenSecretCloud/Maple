...
mod tests {
    ...
    async fn normalized_kimi_k3_id_round_trips_with_its_tool_response() {
        ...
        let requests = transport.requests.lock().expect("request lock");
        assert_eq!(requests.len(), 2, "tool follow-up is a new semantic turn");
    ...
    #[tokio::test]
    async fn does_not_retry_an_sdk_failure_with_unknown_delivery() {
        ...
        assert_eq!(
            ...
            Some(ProviderError::ServerError(
                "Maple's server returned status 503".to_string()
            ))
        );
        assert_eq!(transport.request_count(), 1);
        assert_eq!(transport.remaining_response_count(), 1);
