...
const DEFAULT_TTS_SPEED: f32 = 1.2;
const MIN_TTS_SPEED: f32 = 0.5;
const MAX_TTS_SPEED: f32 = 2.0;

fn resolve_tts_speed(requested: Option<f32>) -> f32 {
    match requested {
...
pub async fn tts_synthesize(
    ...
    let speed = resolve_tts_speed(speed);
    let audio = tts
        .synthesize(&text, &style, 10, speed)
...
mod tests {
    ...
    #[test]
    fn speed_defaults_and_clamps_to_supported_range() {
        assert_eq!(resolve_tts_speed(None), DEFAULT_TTS_SPEED);
        assert_eq!(resolve_tts_speed(Some(0.1)), MIN_TTS_SPEED);
        assert_eq!(resolve_tts_speed(Some(3.0)), MAX_TTS_SPEED);
        assert_eq!(resolve_tts_speed(Some(f32::NAN)), DEFAULT_TTS_SPEED);
    }
