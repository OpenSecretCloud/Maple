    ...
    try {
      try {
        ...
        window.AppleID.auth.init({
          ...
          scope: "name email",
          // TODO: REMOVE AFTER TESTING - Replace with production URL
          // For local development with ngrok
          redirectURI: "https://dea1-37-19-200-146.ngrok-free.app/auth/apple/callback", // Explicitly set ngrok URL
          // In production, use: redirectURI: "https://trymaple.ai/auth/apple/callback",
      ...
      console.log("[Apple Auth] Using redirectURI:", window.location.origin + "/auth/apple/callback");
      ...
      document.addEventListener('AppleIDSignInOnSuccess', async (event) => {
        ...
        try {
          ...
          console.log("[Apple Auth] Success Event:", event);
          ...
          if (event.detail?.data?.authorization?.code) {
            // Standard structure
            code = event.detail.data.authorization.code;
            state = event.detail.data.authorization.state;
            identityToken = event.detail.data.authorization.id_token;
          } else if (event.detail?.authorization?.code) {
          ...
          } else if (event.detail?.code) {
            // Simplified structure
            code = event.detail.code;
            state = event.detail.state;
            identityToken = event.detail.id_token;
          } else if (event.authorization?.code) {
            // Another possible structure
            code = event.authorization.code;
            state = event.authorization.state;
            identityToken = event.authorization.id_token;
      ...
      document.addEventListener('AppleIDSignInOnFailure', (event) => {
        console.error("[Apple Auth] Sign In failed:", event.detail.error);
        if (onError) {
          onError(new Error(event.detail.error || "Apple authentication failed"));
        }
      });
    } catch (error) {
