...
function providerFromCallbackPath(pathname: string): OAuthProvider | null {
  return readProvider(pathname.match(/^\/auth\/(github|google|apple)\/callback$/)?.[1] ?? null);
}
...
function LegacyDesktopOAuthContent() {
  ...
  const callbackProvider = providerFromCallbackPath(pathname);
  if (callbackProvider) {
...
export default function LegacyDesktopOAuthApp() {
  const config = openSecretClientConfig();
  return (
    <LegacyOpenSecretProvider {...config}>
